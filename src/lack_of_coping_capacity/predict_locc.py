"""
Predict future Lack of Coping Capacity (LoCC) under SSP scenarios.

This script:
1. loads the trained XGBoost model
2. loads the already prepared SSP feature table for LoCC
3. creates lagged features within each country-scenario series
4. applies the trained model
5. saves the final LoCC predictions
"""
...
# Already prepared full SSP feature table for LoCC
FEATURE_TABLE_PATH = Path("data/processed/lack_of_coping_capacity/locc_ssp_features_full.csv")

# Trained model and metadata from the final training step
MODEL_PATH = Path("data/models/locc_xgboost.pkl")
METADATA_PATH = Path("data/models/locc_xgboost_metadata.json")

# Final output file with LoCC predictions
PREDICTIONS_OUTPUT_PATH = Path("data/predictions/locc_ssp_predictions.csv")
...
YEAR_START = 2025
YEAR_END = 2100
VALID_SCENARIOS = {"SSP1", "SSP2", "SSP3", "SSP4", "SSP5"}
...
def normalize_scenario(series: pd.Series) -> pd.Series:
    """Standardize scenario names like SSP1, ssp1, SSP 1."""
...
# Exact lagged features expected by the trained model
lagged_feature_cols = metadata["lagged_features"]

# Recover base feature names from lagged names
base_feature_cols = [col.replace("_lag1", "") for col in lagged_feature_cols]
...
require_columns(
    future_wide,
    {"iso3", "scenario", "year"} | set(base_feature_cols),
...
future_wide = future_wide[future_wide["year"].between(YEAR_START, YEAR_END, inclusive="both")].copy()
...
future_lagged, lagged_cols_created = add_lagged_features_by_scenario(
    df_wide=future_wide,
    feature_cols=base_feature_cols,
    lag=1,
)

# Keep only rows where all lagged inputs are available
future_lagged = future_lagged.dropna(subset=lagged_feature_cols).copy()

if future_lagged.empty:
    raise ValueError("No usable future rows left after creating lagged features.")


# _____PREDICT LOCC_____

# Keep lagged feature columns in exact training order
X_future = future_lagged[lagged_feature_cols].copy()

future_lagged["predicted_locc"] = model.predict(X_future)


# _____SAVE PREDICTIONS_____

predictions_df = future_lagged[
    ["iso3", "scenario", "year", "predicted_locc"]
].copy()

PREDICTIONS_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
predictions_df.to_csv(PREDICTIONS_OUTPUT_PATH, index=False)

print("========== FUTURE LOCC PREDICTIONS ==========")
