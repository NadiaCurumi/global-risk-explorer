"""
Validate the hazard prediction models and perform hyperparameter tuning.

This script:
1. Loads the prepared historical hazard dataset in wide format.
2. Tunes Random Forest and XGBoost hyperparameters using a random subset
   of candidate combinations.
3. Compares the tuned models using:
   - rolling temporal validation (train on earlier years, test on the next)
   - grouped spatial cross-validation (train on some countries, test on others)
4. Saves detailed and summary results, plus the best parameter sets.
"""
...
# Input file: already-wide historical hazard dataset
INPUT_PATH = Path("data/processed/hazard/hazard_historical_feature_set.csv")

# Validation outputs
RESULTS_OUTPUT_PATH = Path("data/results/hazard_validation_results.csv")
SUMMARY_OUTPUT_PATH = Path("data/results/hazard_validation_summary.csv")

# Best parameter outputs
RF_BEST_PARAMS_OUTPUT_PATH = Path("data/models/hazard_random_forest_best_params.json")
XGB_BEST_PARAMS_OUTPUT_PATH = Path("data/models/hazard_xgboost_best_params.json")

# Full tuning tables
RF_TUNING_RESULTS_OUTPUT_PATH = Path("data/results/hazard_random_forest_tuning_results.csv")
XGB_TUNING_RESULTS_OUTPUT_PATH = Path("data/results/hazard_xgboost_tuning_results.csv")

# Runtime limits: cap the number of candidates so tuning stays manageable
...
def evaluate_candidate(df_wide: pd.DataFrame, model_factory, model_name: str, params: dict) -> dict:
    ...
    Returns a dictionary with average performance metrics and a ranking score.
    """
...
def tune_model(
    ...
    Returns a DataFrame of tuning results sorted by score descending.
    """
    random.seed(42)
