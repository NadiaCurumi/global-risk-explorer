"""
Explain the final LoCC model with SHAP.

This script:
1. loads the trained LoCC XGBoost model and final training data
2. computes SHAP values
3. saves one SHAP importance table
...
TRAINING_DATA_PATH = Path("data/processed/lack_of_coping_capacity/locc_training_final.csv")
METADATA_PATH = Path("data/models/locc_xgboost_metadata.json")

# Output files
SHAP_IMPORTANCE_OUTPUT_PATH = Path("data/results/locc_shap_importance.csv")
SHAP_BEESWARM_PLOT_PATH = Path("data/results/plots/locc_shap_beeswarm.png")
...
lagged_cols = metadata["lagged_features"]
target_col = metadata["target"]

# Check that all required columns exist
missing_cols = set(lagged_cols + [target_col]) - set(df.columns)
if missing_cols:
...
shap_values_df = pd.DataFrame(shap_values, columns=lagged_cols)
...
# Build final SHAP importance table
...
# _____SHAP IMPORTANCE TABLE_____
...
print(f"Number of features explained: {len(lagged_cols)}")
print("\nTop SHAP features:")
