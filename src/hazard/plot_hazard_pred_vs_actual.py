...
"""
Create predicted vs. actual plots for the Hazard model.
...
TEMP_PLOT_PATH = Path("data/results/plots/hazard_pred_vs_actual_temporal_cv.png")
SPATIAL_PLOT_PATH = Path("data/results/plots/hazard_pred_vs_actual_spatial_cv.png")

TEMP_PRED_PATH = Path("data/results/hazard_temporal_oof_predictions.csv")
SPATIAL_PRED_PATH = Path("data/results/hazard_spatial_oof_predictions.csv")
...
def rolling_temporal_validation_with_predictions(
    ...
    for test_year in years:
        ...
        fold = make_temporal_fold_dataset(
            ...
            cols_to_lag=CONFLICT_FEATURES,   # 🔑 IMPORTANT
        )
...
def plot_pred_vs_actual(pred_df: pd.DataFrame, title: str, output_path: Path):
    ...
    ax.set_xlabel("Actual Hazard score")
